...
angular.module('wfm.users').service('UserClientService',  ['mediator', '$fh' ,function(mediator,$fh) {
  return new UserClient(mediator,$fh);
}]);

//A client for making requests to the cloud side of the user module.
function UserClient(mediator,$fh) {
  var self = this;
  ...
  mediator.subscribe('wfm:user:list', function() {
    self.list($fh, function(error, userList) {
  ...
  mediator.subscribe('wfm:user:create', function(userToCreate) {
    self.create(userToCreate, $fh, function(error, createdUser) {
 ...
 */
UserClient.prototype.list = function list($fh, callback) {
...
//  });
  $fh.cloud({
    "path": config.apiPath, //only the path part of the url, the host will be added automatically
 ...
 * @param callback     - function to be called when the response has returned from the cloud.
 */
UserClient.prototype.create = function create(userToCreate, $fh ,callback) {
...
//  });
  $fh.cloud({
    ...
    "contentType": "application/json",
    "data": userToCreate, //data to send to the server
    "timeout": 25000 // timeout value specified in milliseconds. Default: 60000 (60s)
  }, function(res) {
